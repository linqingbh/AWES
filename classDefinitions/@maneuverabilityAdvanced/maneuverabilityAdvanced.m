classdef maneuverabilityAdvanced
    ...
    properties (SetAccess = private)
        ...
        colorOrder = [228,26,28
            ...
            77,175,74
            152,78,16]./255;
        rgbColors = [255,0,0;0,176,80;0,112,192]./255;
    end
    ...
    methods
        ...
        function obj = maneuverabilityAdvanced(vhcl)
            ...
            ddy = diff(dy,s);
            ddz = diff(dz,s);
            % dot product between kite position and path
            xKdotPath = dot([xK;yK;zK],G_path);
            xKdXP_1stDir = diff(xKdotPath,s);
            xKdXP_2ndDir = diff(xKdXP_1stDir,s);           
            % curvature numerator
    ...
    methods
        ...
        function val = calcMinCurvature(obj)
            [~,val] = fmincon(@(pathParam) obj.eqPathCurvature(obj.aBooth,...
                obj.bBooth,obj.meanElevationInRadians,pathParam,obj.tetherLength),0,[],...
                [],[],[],0,2*pi);
        end
    end
    ...
    methods
        ...
        function val =  plotBodyFrameAxes(obj,azimuth,elevation,...
            ...
            for ii = 1:3
                val(ii) = quiver3(rCM(1),rCM(2),rCM(3),...
                    ...
                    'MaxHeadSize',0.6,...
                    'color',obj.rgbColors(ii,:),...
                    'linewidth',lineWidth);
                hold on
            end
        end
        
        % plot inertial frame axes
        function val =  plotInertialFrameAxes(obj)
            % origin
            O = [0;0;0];
            % axes
            unitAxes = eye(3);
            % plot options
            lineWidth = 0.8;
            scale = obj.tetherLength*0.2;
            % plot
            val = gobjects(3);
            for ii = 1:3
                val(ii) = quiver3(O(1),O(2),O(3),...
                    unitAxes(1,ii),unitAxes(2,ii),unitAxes(3,ii),scale,...
                    'MaxHeadSize',0.6,...
                    'color',obj.rgbColors(ii,:),...
                    'linewidth',lineWidth);
                hold on
        ...
        % plot lemniscate
        function [val,varargout] = plotLemniscate(obj,pathParam)
            % local variable
            if nargin == 1
